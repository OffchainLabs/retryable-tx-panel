...
import { L2ToL1Message, L2TransactionReceipt, getL2Network, getL1Network, L2ToL1MessageStatus, L1Network, L2Network} from "@arbitrum/sdk";
...
export const getL2ToL1Messages = async (txHash: string): Promise<L2ToL1MessageSearchResult> => {
  for (let [chainID, rpcURL] of Object.entries(supportedL2Networks)) {
    ...
    const l1Provider = await new JsonRpcProvider(supportedL1Networks[l1ChainID])
    ...
    if (receipt) {
      ...
      const l2ToL1Messages = await l2Receipt.getL2ToL1Messages(l1Provider, l2Network)
      ...
      for (let l2ToL1Message of  l2ToL1Messages) {
        ...
        const deadlineBlock = status !== L2ToL1MessageStatus.CONFIRMED && status !== L2ToL1MessageStatus.EXECUTED  ? null :  await l2ToL1Message.getFirstExecutableBlock(l2Provider)
        l2MessagesData.push({
          status,
          l2ToL1Message,
          deadlineBlock,
